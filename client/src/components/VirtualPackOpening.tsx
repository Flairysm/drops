...
export function VirtualPackOpening({ packId, packName, onClose }: VirtualPackOpeningProps) {
  ...
  const openPackMutation = useMutation({
    mutationFn: async () => {
      ...
      return response.json() as Promise<VirtualPackResult>;
    },
    onSuccess: (result) => {
      setResult(result);
      ...
      setTimeout(() => {
        ...
        const tierName = hitCard ? tierNames[hitCard.tier as keyof typeof tierNames] : "Common";
            ...
            >
              <div className="p-4 text-center">
                ...
                <div className="text-xs text-muted-foreground">
                  {parseFloat(card.marketValue).toFixed(2)} credits
                </div>
                {card.tier !== "D" && (
