...
interface SpecialPackPopupProps {
  pack: any;
  isOpen: boolean;
  onClose: () => void;
...
export function SpecialPackPopup({ pack, isOpen, onClose, onOpenPack }: SpecialPackPopupProps) {
...
const fetchPackDetails = async () => {
    ...
    try {
      ...
      const response = await apiRequest('GET', `/api/admin/special-packs/${pack.id}?_t=${timestamp}`);
      const data = await response.json();
      console.log('Fetched fresh special pack data:', data);
      setPackData(data);
    } catch (error) {
      console.error('Error fetching special pack details:', error);
      toast({
        title: "Error",
  ...
  const handleAnimationComplete = () => {
    ...
    setPackResult(null);
    // Don't close the dialog - return to the Special pack popup
  };
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Card className="bg-gray-800/50 border border-gray-600/30 shadow-sm">
                <CardContent className="p-3 text-center">
                  ...
                  <div className="text-xs text-gray-400">Available Cards</div>
                </CardContent>
              </Card>
            </div>
          )}
          ...
          <Tabs defaultValue="prizepool" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-gray-800/50 border border-gray-600/30 rounded-lg p-0.5 h-8">
              <TabsTrigger value="prizepool" className="data-[state=active]:bg-purple-500 data-[state=active]:text-white rounded-md transition-all duration-200 text-xs px-2 py-1">
                <Gift className="w-3 h-3 mr-1" />
                Prize Pool
              </TabsTrigger>
              <TabsTrigger value="stats" className="data-[state=active]:bg-purple-500 data-[state=active]:text-white rounded-md transition-all duration-200 text-xs px-2 py-1">
            ...
            {isOpening && (
              ...
              >
                <motion.div
                  ...
                  className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-full p-8 shadow-[0_0_50px_rgba(168,85,247,0.8)]"
                >
                  <motion.div
                    animate={{ rotate: 360 }}
